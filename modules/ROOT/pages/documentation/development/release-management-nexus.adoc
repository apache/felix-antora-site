...
* <<staging-the-release-candidates,Release the module with the Maven Release plugin deploying to the Nexus Staging repository>>
* Check the artifacts
* <<starting-the-vote,Call for vote>>
...
* <<create-an-announcement,Announce the release>>
...
. rollback the version in the `pom.xml` and commit any fixes you need to make+++<div class="info">+++The release manager is free to either reuse the same version when proposing a new release for vote (for example 2.1.2 after a failed 2.1.2 release attempt), or choose a different number (for example 2.1.4 after a failed 2.1.2 attempt).
(cf.
vote on 02/14/11).+++</div>+++
...
* <<release-bundle-to-the-obr,Release Bundles to the OBR>>
