...
* link:{{ refs.-overview.path }}[OSGi Launching and Embedding API Overview] ** link:{{ refs.-creating-and-configuring.path }}[Creating and Configuring the Framework Instance] ** link:{{ refs.-starting-instance.path }}[Starting the Framework Instance] ** link:{{ refs.-stopping-instance.path }}[Stopping the Framework Instance]
* link:{{ refs.-launching.path }}[Launching Felix] ** link:{{ refs.-standard-launcher.path }}[Standard Felix Launcher] ** link:{{ refs.-custom-launcher.path }}[Custom Felix Launcher]
* link:{{ refs.-embedding.path }}[Embedding Felix] ** link:{{ refs.-host-interaction.path }}[Host/Felix Interaction] ** link:{{ refs.-host-services.path }}[Providing Host Application Services] ** link:{{ refs.-host-service-usage.path }}[Using Services Provided by Bundles] _** link:{{ refs.-service-reflection.path }}[Using Bundle Services via Reflection] **_ link:{{ refs.-service-other.path }}[Other Approaches]
* link:{{ refs.-caveat.path }}[Caveat]
...
The configuration map may contain any of the framework configuration properties listed in the link:{{ refs.apache-felix-framework-configuration-properties.path }}[Apache Felix Framework Configuration Properties] document, not the launcher configuration properties.
...
if you are upgrading from an earlier version, the link:{{ refs.apache-felix-framework-configuration-properties-migrating.path }}[configuration property document] describes the configuration property changes.+++</div>+++
...
see the link:{{ refs.apache-felix-framework-usage-documentation-configuring-felix.path }}[usage document] for more information on configuring the Felix framework and on the various configuration properties.
...
see the link:{{ refs.-custom-launcher.path }}[custom launcher example] below to see how the META-INF/services `FrameworkFactory` is obtained.
. {blank}
+
[cols=2*]
|===
...
see the usage document for more information link:{{ refs.apache-felix-framework-usage-documentation-configuring-framework.path }}[configuration properties] and [bundle auto-deploy
...
In the section on link:{{ refs.-launching.path }}[launching] the framework above, the `Felix` class accepts a configuration property called `felix.systembundle.activators`, which is a list of bundle activator instances.
...
Providing services from the host application to bundles inside the embedded Felix framework instance follows the basic approach laid out in link:{{ refs.-host-interaction.path }}[above].
